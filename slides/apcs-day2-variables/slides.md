...
記憶體裡面就是一堆 0 跟 1，本身沒有意義。型態的作用是告訴電腦：這塊資料佔多大、要怎麼解讀。

同樣四個位元組，當成整數看是一個數字，當成別的東西看就完全不一樣。

打個比方，同樣一串數字 0912345678，你說它是電話號碼，別人就會拿去打；你說它是帳號，別人就會拿去轉帳。數字本身一樣，怎麼解讀決定了它是什麼。

型態選對還有一個好處：不浪費記憶體，而且可以避免一些很奇怪的錯誤。等一下講到整數除法的時候會詳細說明
-->
...
這四個盒子大小不一樣、裝的東西也不一樣。`int` 四個位元組、`double` 八個、`char` 一個、`string` 不固定。

`string` 為什麼不固定？因為文字可以很短也可以很長，「A」跟一整段文章都是文字，沒辦法事先講死要多大。

位元組的數字不用背，有印象就好。你只要記得一件事：型態決定了盒子多大、還有裡面的 0 跟 1 要怎麼解讀。
-->
