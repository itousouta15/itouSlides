...
這個差別等一下第 6 題會直接用到。那題要判斷使用者輸入的是哪個運算符號，要寫 `op == '+'`，單引號。如果你寫成雙引號會編譯錯誤，因為型態對不起來。
-->
...
<!--
（過場）資料型態學完了，先用兩題基礎題練手感——完全不用碰運算子，純粹練型態宣告跟輸入輸出。
-->
...
<div class="grid grid-cols-2 gap-4 my-4 font-mono text-sm">
  <div class="border border-gray-400 border-opacity-40 p-3">
    <div class="opacity-60 text-xs mb-1">輸入</div>Amy<br/>12<br/>B
  </div>
  <div class="border border-gray-400 border-opacity-40 p-3">
    <div class="opacity-60 text-xs mb-1">輸出</div>12 號 Amy（B 組）
  </div>
</div>
...
給你姓名、座號、分組代號三筆資料，照格式印出一張點名卡。

這題還沒用到任何運算子，純粹練剛剛學的型態宣告跟 cin / cout。重點是看到不同資料要能馬上反應該用哪個型態。
-->
...
三行輸入，姓名是字串、座號是整數、分組代號是單一字母，剛好對應 string、int、char。

輸出就是把這三個值套進固定格式印出來。

這題完全不用算東西，重點只有兩件事：型態讀對、格式排對。
-->
...
`string name`、`int seat`、`char group`，三種型態各出現一次。`cin` 一樣照順序讀進去，型態不同沒關係，昨天講過 `cin` 會自動跳過空白跟換行。

輸出那行照著範例的格式排：座號、「號」、姓名、括號、分組、「組」，特別提醒括號要用全形（），不是英文鍵盤打的半形()，兩個字看起來像，判題是逐字比對，打錯就 WA。
-->
...
打開 [oj.itousouta.me → 課程 Day2 → 第 2 題](https://oj.itousouta.me/problems/30)
...
<div class="grid grid-cols-2 gap-4 my-4 font-mono text-sm">
  <div class="border border-gray-400 border-opacity-40 p-3">
    <div class="opacity-60 text-xs mb-1">輸入</div>37
  </div>
  <div class="border border-gray-400 border-opacity-40 p-3">
    <div class="opacity-60 text-xs mb-1">輸出</div>98.6
  </div>
</div>
...
輸入攝氏溫度，套公式換算成華氏溫度印出來。

這題會用到 `double`——因為溫度可能有小數點——還有加減乘除幾個最基本的運算子，但完全不用比較或邏輯運算子，也不用 if。
-->
...
輸入是一個實數 C，代表攝氏溫度，可能有小數點，像人體體溫 36.5 度這種。

輸出是換算後的華氏溫度。

處理就是套公式，算完直接印出來，不用做任何判斷。

關鍵在型態：C 要宣告成 `double`。如果貪方便宣告成 `int`，遇到 36.5 這種輸入會讀壞——這就是回到今天一開始講的：型態要跟資料的長相對起來。
-->
...
`c`、`9`、`5`、`32` 混在一起算，只要**其中一個**是 `double`（這裡是 `c`），整條算式就會自動用小數計算，不用每個數字都額外轉型。

</div>

<div class="mt-2 border-l-4 border-yellow-500 pl-4 text-sm">
...
`double c` 讀進攝氏溫度，套公式算出 `f`，直接印出來。

注意公式裡 `9` 和 `5` 都寫成整數字面量，沒有特別轉型，但因為 `c` 本身是 `double`，整條算式會自動用小數運算，不用每個數字都寫成 `9.0`、`5.0`。這個「只要有一邊是小數就好」的規則，等一下 BMI 那題還會再遇到一次。

還有一個小提醒：如果算出來的 F 剛好是整數，`cout` 印出來不會多一個 `.0`。像 C 等於 0 的時候，F 是 32，印出來就是 `32`，不是 `32.0`。這是正常的，不是算錯。
-->
...
打開 [oj.itousouta.me → 課程 Day2 → 第 3 題](https://oj.itousouta.me/problems/31)
...
如果自己電腦上跑出來的 F 是整數但沒有 `.0`，那是正常的，`cout` 印 double 本來就會把多餘的 0 砍掉，不用懷疑自己哪裡算錯。
-->
...
第二，判斷奇偶。`n % 2 == 0` 是偶數，不成立就是奇數。這個等一下第 4 題馬上用到。
...
打開 [oj.itousouta.me → 課程 Day2 → 第 4 題](https://oj.itousouta.me/problems/2)
...
打開 [oj.itousouta.me → 課程 Day2 → 第 5 題](https://oj.itousouta.me/problems/3)
...
今天的題目不會特別考這個，但等一下第 6 題處理除以 0 的時候會出現——先判斷是不是除法，是的話再判斷除數是不是 0。先看過有印象就好。
-->
...
<!--
第 6 題，簡易計算機。今天最綜合的一題，會用到所有運算子加完整的 if / else if。
...
打開 [oj.itousouta.me → 課程 Day2 → 第 6 題](https://oj.itousouta.me/problems/4)
...
<!--
第 7 題，BMI 分級。今天最後一題，也是整數除法陷阱的實戰。
...
打開 [oj.itousouta.me → 課程 Day2 → 第 7 題](https://oj.itousouta.me/problems/5)
